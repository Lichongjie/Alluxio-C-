...
Status FileSystem::GetMountTable(
  ...
  try {
        jobject jMountTable =  JniHelper::CallObjectMethod(
        FileSystem::filesystem, "alluxio/client/file/FileSystem", "getMountTable",
        "java/util/Map");
    ...
    int mapSize = JniHelper::CallIntMethod(jMountTable, "java/util/Map",
        ...
        for(int i = 0; i < mapSize; i ++) {
            jobject keyItem = JniHelper::GetEnv()->GetObjectArrayElement((jobjectArray) keyArray, i);
            JniHelper::CacheClassName(keyItem, "java/lang/Object");
            std::string key = JniHelper::JstringToString((jstring)keyItem);
            jobject valueItem = JniHelper::CallObjectMethod(jMountTable, "java/util/Map",
